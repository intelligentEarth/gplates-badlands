    ...
    def run(self):
        ...
        for i in range(samples-1):
            ...
            if u < mh_prob: # Accept sample
                ...
                print('ACCEPTED by - ','Chain ',int(self.temperature),' Sample :', i, ' Likl ', likelihood , avg_rmse_el, avg_rmse_er, '   --------- ')
            ...
            with file(('%s/posterior/pos_parameters/stream_chain_%s.txt' % (self.folder, self.temperature)),'a') as outfile:
                np.savetxt(outfile,np.array([pos_param[i+1,:]]))
            with file(('%s/posterior/pos_likelihood/stream_chain_%s.txt' % (self.folder, self.temperature)),'a') as outfile:
...
def main():
    ...
    elif problem == 2: # 150 MILLION YEARS
